...
class Wechat
{
	...
	const MEDIA_UPLOAD_IMG = '/media/uploadimg?';
...
//		}
////		\Think\Log::write('checkAuth1');
//		if ($token) { //手动指定token，优先使用
//		    $this->access_token=$token;
//		    return $this->access_token;
//		}
//		$authname = 'wechat_access_token'.$appid;
////		\Think\Log::write('checkAuth2');
//		if ($rs = $this->getCache($authname))  {
//			$this->access_token = $rs;
//			return $rs;
//		}
////		\Think\Log::write('checkAuth3');
//		$result = $this->http_get(self::API_URL_PREFIX.self::AUTH_URL.'appid='.$appid.'&secret='.$appsecret);
//		if ($result)
//		{
////			\Think\Log::write('checkAuth4 '.($result));
...
//		}
	...
	/**
	 * 上传图文消息内的图片获取URL(认证后的订阅号可用)
	 * @param array $data {"media":'@Path\filename.jpg'}
	 * @return boolean|array
	 */
	public function uploadImg($data){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_UPLOAD_IMG.'access_token='.$this->access_token,$data,true);
		if($result){
			$json = json_decode($result,true);
			return $json['url'];
		}
